...
class VanityPoolParserTest extends FunSuite with Matchers with Inside {
  ...
  test("should be able to parse well-formed work output") {
    ...
    inside(parsedWork(0)) { case ParsedWorkItem(pattern, publicKey, networkByte, reward) =>
    ...
    inside(parsedWork(1)) { case ParsedWorkItem(pattern, publicKey, networkByte, reward) =>
    ...
    inside(parsedWork(2)) { case ParsedWorkItem(pattern, publicKey, networkByte, reward) =>
